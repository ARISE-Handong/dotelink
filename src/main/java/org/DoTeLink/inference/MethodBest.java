...
public class MethodBest {
	...
	public static JsonArray relate(JsonArray jSegProductionMethod, JsonArray jSegTestMethod, CompilationUnit testCodeCu) {
		...
		List<Link> sentenceWiseLinks = Inference.inferSentenceWiseBest(productionMatrix);
		List<Link> tcsWiseLinks = Inference.inferTestCodeSnippetWiseBest(productionMatrix);

		List<Link> links = Union(sentenceWiseLinks, tcsWiseLinks);
